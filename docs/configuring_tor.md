...
eacrd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, eacrd supports a hybrid,
...
Configuring eacrd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the eacrd command
line or in the eacrd configuration file.  Typically the Tor proxy address will be
...
By default, eacrd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since eacrd will act as the server.
...
```bash
$ ./eacrd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
eacrd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the eacrd command line or in the eacrd
...
```bash
$ ./eacrd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
eacrd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./eacrd --proxy=127.0.0.1:9050 --torisolation
```
