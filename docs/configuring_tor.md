...
ecrd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, ecrd supports a hybrid,
...
Configuring ecrd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the ecrd command
line or in the ecrd configuration file.  Typically the Tor proxy address will be
...
By default, ecrd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since ecrd will act as the server.
...
```bash
$ ./ecrd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
ecrd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the ecrd command line or in the ecrd
...
```bash
$ ./ecrd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
ecrd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./ecrd --proxy=127.0.0.1:9050 --torisolation
```
