...
import (
	...
	"github.com/Eacred/eacrd/rpc/jsonrpc/types"
)
...
var helpDescsEnUS = map[string]string{
	...
	"debuglevel--synopsis": "Dynamically changes the debug logging level.\n" +
		...
		"The valid subsystems are AMGR, ADXR, BCDB, BMGR, ECRD, CHAN, DISC, PEER, RPCS, SCRP, SRVR, and TXMP.\n" +
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (eacrd extension)",
	...
	"getrawmempoolverboseresult-fee":              "Transaction fee in eacred",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (eacrd does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown eacrd.",
	"stop--result0":  "The string 'eacrd stopping.'",
