...
var helpDescsEnUS = map[string]string{
	...
	"getblock-verbosetx":   "Specifies that each transaction is returned as a JSON object and only applies if the verbose flag is true (ecrd extension)",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (ecrd does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown ecrd.",
	"stop--result0":  "The string 'ecrd stopping.'",
