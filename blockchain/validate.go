...
func CheckTransactionSanity(tx *wire.MsgTx, params *chaincfg.Params) error {
	...
	// Eacred is a quantity of atoms as defined by the AtomsPerCoin constant.
	//
	...
	for txOutIdx, txOut := range tx.TxOut {
		...
		// detected and reported.  This is impossible for Eacred, but perhaps
...
//
// Eacred: Check the stake transactions to make sure they don't have this txid
// too.
...
// owner of the Eacred and therefore allowed to spend them.  As it checks the
...
func CheckTransactionInputs(subsidyCache *standalone.SubsidyCache, tx *dcrutil.Tx, txHeight int64, view *UtxoViewpoint, checkFraudProof bool, chainParams *chaincfg.Params) (int64, error) {
	...
	// -------------------------------------------------------------------
	// Eacred stake transaction testing.
	// -------------------------------------------------------------------
	...
	// -------------------------------------------------------------------
	// Eacred general transaction testing (and a few stake exceptions).
	// -------------------------------------------------------------------
	...
	for idx, txIn := range msgTx.TxIn {
		...
		// Eacred is a quantity of atoms as defined by the AtomPerCoin
		// constant.
