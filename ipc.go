...
// processes to communicate with and manage the lifetime of a eacrd child
...
// message, shutdown begins.  This prevents eacrdd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			eacrddLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child eacrdd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	eacrddLog.Errorf("Failed to write to pipe: %v", err)
}
