...
// processes to communicate with and manage the lifetime of a ecrd child
...
// message, shutdown begins.  This prevents ecrd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			ecrdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child ecrd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	ecrdLog.Errorf("Failed to write to pipe: %v", err)
}
