...
// message, shutdown begins.  This prevents eacrd from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			eacrdLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child eacrd process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	eacrdLog.Errorf("Failed to write to pipe: %v", err)
}
