...
// nodeConfig contains all the args, and data required to launch a ecrd process
...
// arguments returns an array of arguments that be used to launch the ecrd
// process.
...
// command returns the exec.Cmd which will be used to start the ecrd process.
...
// to the ecrd process that is launched via Start().
...
// as the base for the log and data directories for ecrd.
...
// start creates a new ecrd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "ecrd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running ecrd process, and waits until it exits
...
// shutdown terminates the running ecrd process, and cleans up all
