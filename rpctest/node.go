...
// nodeConfig contains all the args, and data required to launch a eacrd process
...
// arguments returns an array of arguments that be used to launch the eacrd
// process.
...
// command returns the exec.Cmd which will be used to start the eacrd process.
...
// to the eacrd process that is launched via Start().
...
// as the base for the log and data directories for eacrd.
...
// start creates a new eacrd process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "eacrd.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running eacrd process, and waits until it exits
...
// shutdown terminates the running eacrd process, and cleans up all
