...
// Harness fully encapsulates an active ecrd process to provide a unified
// platform for creating rpc driven integration tests involving ecrd. The
// active ecrd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active ecrd process is fully
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the ecrd node itself. This spawns a new process which will be
	// managed
	...
	// Ensure ecrd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created ecrd
